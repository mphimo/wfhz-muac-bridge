...
### Time-based split ----
pred_flawless_time <- Test_flawless_time |> 
  summarise(
    n = n(),
    ...
    pred_wfhz = predict_prevalence(
      obs_prev = prev_muac, 
      known_prop_wfhz = features_flawless_time$median_prop_wfhz, 
      known_prop_muac = features_flawless_time$median_prop_muac,
      .for = "wfhz"
    ),
...
### Random-based split ----
pred_flawless_random <- Test_flawless_random |> 
  summarise(
    n = n(),
    prev_wfhz = mean(gam_wfhz, na.rm = TRUE) * 100,
    prev_muac = mean(gam_muac, na.rm = TRUE) * 100,
