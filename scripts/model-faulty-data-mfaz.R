...
### Time-based split ----
mfaz_wfhz_pred_flt_time <- mfaz_wfhz_flt_test_time |> 
  summarise(
    n = n(),
    ...
    pred_wfhz = predict_prevalence(
      obs_prev =  prev_mfaz, 
      known_prop_wfhz = mfaz_wfhz_feat_flt_time$median_prop_wfhz, 
      known_prop_muac = mfaz_wfhz_feat_flt_time$median_prop_mfaz,
      .for = "wfhz"
    ),
...
### Estimate model accuracy: MAE and MAPE ----
  ...
  mfaz_wfhz_flt_time_model_accuracy <- mfaz_wfhz_pred_flt_time |> 
    summarise(
...
### Random-based split ----
mfaz_wfhz_pred_flt_rdm <- mfaz_wfhz_flt_test_rdm |> 
  summarise(
    n = n(),
    prev_wfhz = mean(gam_wfhz, na.rm = TRUE) * 100,
    prev_mfaz = mean(gam_mfaz, na.rm = TRUE) * 100,
    pred_wfhz = predict_prevalence(
      obs_prev =  prev_mfaz, 
      known_prop_wfhz = mfaz_wfhz_feat_flt_rdm$median_prop_wfhz, 
      known_prop_muac = mfaz_wfhz_feat_flt_rdm$median_prop_mfaz,
      .for = "wfhz"
    ),
    pred_mfaz = predict_prevalence(
      obs_prev = prev_wfhz, 
...
### Estimate model accuracy: MAE and MAPE ----
  ...
  mfaz_wfhz_flt_rdm_model_accuracy <- mfaz_wfhz_pred_flt_rdm |> 
    summarise(
      mae = mae(actual = prev_wfhz, predicted = pred_wfhz),
      ratio_mae = mae(actual = prev_wfhz, predicted = pred_wfhz_ratio),
      mape = mape(actual = prev_wfhz, predicted = pred_wfhz) * 100,
